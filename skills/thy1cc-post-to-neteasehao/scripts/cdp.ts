...
export function findChromeExecutable(): string | undefined {
  const override = process.env.NETEASEHAO_BROWSER_CHROME_PATH?.trim();
...
export function getDefaultProfileDir(): string {
  ...
  return path.join(base, 'neteasehao-browser-profile');
}
...
export class CdpConnection {
  ...
  private nextId = 0;
  private pending = new Map<number, { resolve: (value: unknown) => void; reject: (error: Error) => void; timer: ReturnType<typeof setTimeout> | null }>();
  ...
  private constructor(ws: WebSocket) {
    ...
    this.ws.addEventListener('message', (event) => {
      try {
        ...
        const payload = JSON.parse(data) as { id?: number; result?: unknown; error?: { message?: string } };
...
export async function findExistingChromeDebugPort(): Promise<number | null> {
  ...
  try {
    ...
    const ports = output.split('\n').map((value) => parseInt(value, 10)).filter((value) => Number.isFinite(value) && value > 0);
...
export async function launchChrome(url: string, profileDir?: string): Promise<{ cdp: CdpConnection; chrome: ReturnType<typeof spawn> }> {
  ...
  if (!chromePath) throw new Error('Chrome not found. Set NETEASEHAO_BROWSER_CHROME_PATH if needed.');
...
export async function getPageSession(cdp: CdpConnection, urlPattern: string): Promise<ChromeSession> {
  ...
  const attached = await cdp.send<{ sessionId: string }>('Target.attachToTarget', { targetId: pageTarget.targetId, flatten: true });
  ...
  return { cdp, sessionId: attached.sessionId, targetId: pageTarget.targetId };
}
